...
def get_parser():
    ...
    parser.add_argument('--model_id', default='gref_google', help='name to identify the model')
    ...
    parser.add_argument('--lr', default=0.000035, type=float, help='the initial learning rate')
    ...
    parser.add_argument('--output-dir', default='./checkpoints_CDC2/', help='path where to save checkpoint weights')
    parser.add_argument("--local_rank", default=0, type=int, help='local rank for DistributedDataParallel')
    parser.add_argument('--device', default='cuda:0', help='device')  # only used when testing on a single machine
    # parser.add_argument('--resume', help='resume from checkpoint', default="/home/zy/zyq/LAVT2/checkpoints2/model_best_gref_google.pth") #不重新训练
    ...
    parser.add_argument('--pretrained_swin_weights',default='./pretrained_weights/swin_base_patch4_window12_384_22k.pth', help='path to pre-trained Swin backbone weights')
    parser.add_argument('--ddp_trained_weights', action='store_true',
                       help='Only needs specified when testing,'
                           'whether the weights to be loaded are from a DDP-trained model')
    parser.add_argument('--window12', default=True, action='store_true',
                        help='only needs specified when testing,'

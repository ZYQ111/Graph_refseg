...
class Conv2d_cd(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding=0, dilation=1, groups=1, theta=0.5):

        super(Conv2d_cd, self).__init__() 
        self.conv = nn.Conv2d(in_channels, 64, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups)
        self.conv2 = nn.Conv2d(in_channels, 64, kernel_size=3, stride=1, padding=1, dilation=dilation, groups=groups)
        self.pool = nn.MaxPool2d(4, 4)
       
        self.theta = theta

    def forward(self, x):
        out_normal1 = self.conv(x)
        out_normal2 = self.conv2(x)
        [C_out,C_in, kernel_size,kernel_size] = self.conv2.weight.shape
        kernel_diff = self.conv2.weight.sum(2).sum(2)
        ...
        out_normal2 = out_normal2 - self.theta * out_diff
        aaa = self.pool(out_normal2)
        bbb = torch.cat([out_normal1,aaa],dim = 1)
        return bbb
        
        # 差分卷积
        # bbb = torch.cat([out_normal1,aaa],dim = 1)
        # out_normal2 = self.pool(out_normal2)
        # out_normal = torch.cat([out_normal1,out_normal2],dim = 1)
        # alpha = 0.5
        ...
        # else:
            ...
            # kernel_diff2 = self.conv2.weight.sum(2).sum(2)
            # kernel_diff = torch.cat([kernel_diff1,kernel_diff2],dim = 0)
            # kernel_diff = kernel_diff[:, :, None, None]
            # print("kernel_diff1")
            # print(kernel_diff1.shape)
            # print("kernel_diff")
            ...
            # out_diff = F.conv2d(input=x, weight=kernel_diff, bias=torch.cat([self.conv.bias,self.conv2.bias]), stride=self.conv.stride, padding=0, groups=self.conv.groups)
            # out_diff = F.conv2d(input=x, weight=kernel_diff, bias=self.conv2.bias, stride=self.conv2.stride, padding=0, groups=self.conv.groups)
            
            # aaa = out_normal2 - self.theta * out_diff
            
            
            
            # print(torch.cat([self.conv.bias,self.conv2.bias]))
            # return out_normal - self.theta * out_diff  
            # return alpha*(out_normal - self.theta * out_diff) + (1-alpha) * out_normal    
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.proj = Conv2d_cd(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, theta=0.5)
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # B C Wh Ww [1, 128, 120,120]

        # for i in range(120):
        #     img22 = x[:,i,::].reshape(120,120).cpu().numpy()
        #     plt.imsave("/home/zy/zyq/LAVT2/keshihua/1/"+str(i)+".png",img22)
...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        for i in range(self.num_layers):
            ...
            # x_out, H, W, x, Wh, Ww = layer(Wh, Wh, Ww, l, l_mask)
...
class PWAM(nn.Module):
    ...
    def forward(self, x, l, l_mask):
       ...
       # vis_t = self.g(vis)
       # lang_t = self.g(lang)
       # vis_trans = vis_t.permute(0, 2, 1)
       # lang_trans = lang_t.permute(0, 2, 1)
       # similar = torch.matmul(vis_trans, lang_t)
       # similar2 = F.softmax(similar, dim=2)
       # embedding = torch.matmul(similar2, lang_trans).permute(0, 2, 1) 
       # embedding = self.project_mm(embedding)
       # embedding = embedding.permute(0, 2, 1)
        # mm = torch.mul(vis, lang)
        # mm = self.project_mm(mm)
        # mm = mm.permute(0, 2, 1)
        # mm_all = torch.add(mm,embedding)
       # mm_all = embedding

        # 第1次改动
        # vis_trans = vis.permute(0, 2, 1)
        # lang_trans = lang.permute(0, 2, 1)
        # similar = torch.matmul(vis_trans, lang)  # eij=x[B,w*h,dim]*z[B,dim,w*h]
        # similar2 = F.softmax(similar, dim=2)
        # embedding = torch.matmul(similar2, lang_trans).permute(0, 2, 1) # (B, dim, H*W)
        # embedding = self.project_mm(embedding)
        # embedding = embedding.permute(0, 2, 1)
        # mm = torch.mul(vis, lang)
        ...
        # print("long_g_trans   ", lang_g_trans.shape)
        # print("vis_g_trans   ", vis_g_trans.shape)
        # print("similar   ", similar.shape)
        # print("similar2   ", similar2.shape)
